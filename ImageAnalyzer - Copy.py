...
class ImageAnalyzer:
    ...
    def reset_image_state(self, reset_zoom=True):
        ...
        if not self.keep_calibration_dots_fixed:
            ...
            self.calibration_done = False
            self.calibration_factor = 1.0
        # Always reset these other modes/data
        self.artery_dots = []
        self.line_points = []
        self.measurements = []
        self.line_measurements = []
        self.line_measurement_points = []
        # self.calibration_done = False # Reset handled above conditionally
        # self.calibration_factor = 1.0 # Reset handled above conditionally
    ...
    def on_press(self, event):
        ...
        if self.edge_selection_mode: # Legacy FIND_EDGES ROI start
            ...
            if len(self.angle_points) >= 3: # A full angle is on screen. Reset and start new.
                self.angle_points = []
                # The new angle measurement will be added, but previous ones from this session remain
                # until "Reset Angle" is clicked.
                self.measurement.set("Angle: Previous angle cleared. Click first point.")
            ...
            if len(self.angle_points) == 1:
                ...
                if mag1_sq < 1e-9 or mag2_sq < 1e-9:
                    ...
                    self.update_tables()
                    # angle_points are intentionally not cleared to keep visualization
            current_mode_action = True
    ...
    def reset_calibration(self, ask_confirm=True):
        ...
        # Also reset the 'keep fixed' flag and button state
        self.keep_calibration_dots_fixed = False
        if "Keep Dots Fixed" in self.buttons:
            try: self.buttons["Keep Dots Fixed"].config(relief=tk.RAISED)
            except tk.TclError: pass

        if self.calibration_mode:
            self._reset_all_modes() # This will set the status message
        else:
    ...
    def reset_angle_mode(self):
        """Resets the points and measurements for Angle Mode."""
        self.save_state()
        self.angle_points = []
        # Remove ALL angle measurements from the main list
        initial_meas_count = len(self.measurements)
        self.measurements = [m for m in self.measurements if m.get("type") != "angle"]
        
        if len(self.measurements) < initial_meas_count:
             self.measurement.set("Status: Angle measurements cleared.")
        else:
             self.measurement.set("Status: No angles to clear.")

        if self.angle_mode: # If we are in angle mode, stay in it but ready for new points
            self.measurement.set("Angle: Click first point.")
    ...
    def save_state(self):
        ...
        state = {
            ...
            "keep_calibration_dots_fixed": self.keep_calibration_dots_fixed, # Added flag
        }
    ...
    def _restore_state(self, state):
         ...
         self.global_canny_active = state.get("global_canny_active", False)
         self.keep_calibration_dots_fixed = state.get("keep_calibration_dots_fixed", False) # Added flag restore

         # Update button states based on restored flags
         if "Global Canny" in self.buttons:
             try: self.buttons["Global Canny"].config(relief=tk.SUNKEN if self.global_canny_active else tk.RAISED)
             except tk.TclError: pass
         if "Keep Dots Fixed" in self.buttons:
             try: self.buttons["Keep Dots Fixed"].config(relief=tk.SUNKEN if self.keep_calibration_dots_fixed else tk.RAISED)
             except tk.TclError: pass
    ...
    def undo(self, event=None):
        ...
        current_state = {
            ...
            "global_canny_active": self.global_canny_active,
            "keep_calibration_dots_fixed": self.keep_calibration_dots_fixed, # Added flag
        }
    ...
    def toggle_keep_dots_fixed(self):
        """Toggles the flag to keep calibration dots fixed when changing images."""
        self.save_state()
